import java.util.*;
import java.util.InputMismatchException; // me lo scrivo solo per ricordarmelo
...
public class ArrayAlgsTester{
	public static void main(String[] args){
		...
		while(!check1){
			try{
				System.out.println("Inserisci dimensione dell array:");
				dimensione = scan.nextInt();
				if(dimensione > 0){
					check1 = true;
				}
				else{
					System.out.println("Inserisci quantita' positiva");
				}
				
			}
			catch(InputMismatchException e){
				System.out.println("Inserisci elemento valido");
				scan.nextLine();
			}
		}
		while(!check2){
			try{
				System.out.println("Inserisci variabilita' dei numeri dell array:");
				variabilita = scan.nextInt();
				if(variabilita > 0){
					check2 = true;
				}
				else{
		...
		do{
			System.out.println("Premi S per ordinare array con algoritmo SelectionSort" + "\n" + "Premi M per ordinare array con algoritmo MergeSort" + "\n" + "Premi I per ordinare array con algoritmo InsertionSort" + "\n" + "Premi Q per terminare il programma");
			comando1 = scan.next().toUpperCase().charAt(0);
			if(comando1 == 'S'){
				ArrayAlgs pippo = new ArrayAlgs();
				int[] v = pippo.randomIntArray(dimensione, variabilita);
				String arrayNonOrdinato = pippo.printArray(v, v.length);
				System.out.println("Array non ordinato:");
				System.out.println(arrayNonOrdinato);
				pippo.selectionSort(v, v.length);
				String arrayOrdinato = pippo.printArray(v, v.length);
				System.out.println("Array ordinato:");
				System.out.println(arrayOrdinato);
				do{
					System.out.println("Premi L per cercare un risultato con linearSearch" + "\n" + "Premi B per cercare un risultato con binarySearch" +"\n" + "Premi Q per terminare il programma");
					comando2 = scan.next().toUpperCase().charAt(0);
					check3 = false;
					if(comando2 == 'L'){
						while(!check3){
							try{
								System.out.println("Inserisci elemento da cercare:");
								trovare = scan.nextInt();
								if(trovare > 0){
									check3 = true;
								}
								else{
						...
						int indice = pippo.linearSearch(v, v.length, trovare);
						if(indice == -1){
							System.out.println("Elemento non trovato");
						}
						else{
							System.out.println("Elemento trovato in posizione: " + "\n" + indice);
						}
					}
					else if(comando2 == 'B'){
						while(!check3){
						...
						int indice = pippo.binarySearch(v, v.length, trovare);
						if(indice == -1){
							System.out.println("Elemento non trovato");
						}
						else{
				...
				System.out.println("Array non ordinato:");
				System.out.println(arrayNonOrdinato);
				pippo.insertionSort(v, v.length);
				String arrayOrdinato = pippo.printArray(v, v.length);
				System.out.println("Array ordinato:");
				System.out.println(arrayOrdinato);
				do{
